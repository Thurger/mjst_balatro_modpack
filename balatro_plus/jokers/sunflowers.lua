...
local localization = {
    ...
    text = {
        ...
        "scoring {C:attention}non-Clubs{} card",
...
local function calculate(self, context)
    ...
    if context.individual and context.cardarea == G.play then
        if self.ability.name == (MOD_ID .. "_ability_" .. CARD_NAME) and not context.blueprint then
            ...
            elseif not context.other_card:is_suit("Clubs") then
                current_mult = current_mult - 1
            end
...
SMODS.Jokers.j_mjst_mod_sunflowers.calculate = calculate