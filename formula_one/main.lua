...
-- local name = "your_collection_" .. MOD_ID
...
--             local center = G.P_CENTER_POOLS[G.FUNCS["your_collection_" .. name .. "_page"] = function(args)
--     if not args or not args.cycle_config then return end
...
--     for j = 1, #G.your_collection do
--         for i = 1, nb_item_per_row_table[j] do
--             local center = G.P_CENTER_POOLS["Enhanced"][(j-1) * 3 + i + (2*(args.cycle_config.current_option - 1))]
...
-- end][(j-1) * 3 + i + (2*(args.cycle_config.current_option - 1))]
--             if not center then break end
--             local card = Card(G.your_collection[j].T.x + G.your_collection[j].T.w/2, G.your_collection[j].T.y, G.CARD_W, G.CARD_H, G.P_CARDS.empty, center)
--             card:start_materialize(nil, i>1 or j>1)
--             G.your_collection[j]:emplace(card)
...
--     end
--     _G[name .. "_options"] = {}
--     for i = 1, math.ceil(#G.P_CENTER_POOLS["Enhanced"]/6) do
--         table.insert(_G[name .. "_options"], localize('k_page')..' '..tostring(i)..'/'..tostring(math.ceil(#G.P_CENTER_POOLS["Enhanced"] / 6)))
--     end

--     for j = 1, #G.your_collection do
--         for i = 1, 4 do
--             local center = G.P_CENTER_POOLS["Enhanced"][(j-1) * 3 + i]
